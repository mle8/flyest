...
which ufasta >/dev/null || error_exit "ufasta not found!"
[[ ! -e $MYPATH/fix_consensus_from_vcf.pl ]] && error_exit "Missing fix_consensus_from_vcf.pl in $MYPATH"
...
if [ ! -e "$ASMPATH/$BASM.fix.success" ] && [ $FIX -gt 0 ]; then
    ...
    (
        ...
        echo "$MYPATH/fix_consensus_from_vcf.pl <($MYPATH/ufasta extract -f $ASMPATH/$BASM.names $ASMPATH/$BASM) < ../$BASM.vcf > \$1.fixed.tmp 2>\$1.fixed.err && mv \$1.fixed.tmp \$1.fixed && touch \$1.fix.success" >> commands.sh
        echo "fi" >> commands.sh
        chmod 0755 commands.sh
